...
class ImitationGymEnv(quadruped_gym_env.QuadrupedGymEnv):
  ...
  def __init__(self,
    ...
    if self._task_env == "FULL_TRAJ":
      ...
      action_repeat = int(FLIGHT_TIME_START / (time_step * task_env_num )) # 100
    elif self._task_env == "PARTIAL_TRAJ":
      action_dim = 12 * 2 #( FLIGHT_TIME_START / time_step * action_repeat )
      action_repeat = 50
      # action_dim = 12 * 5 #( FLIGHT_TIME_START / time_step * action_repeat )
      # action_repeat = 20
  ...
  def step_partial_traj(self, action):
    ...
    # reshaped_action = np.reshape(action, (12, 5))
    ...
    info = {}
    info[1] = self._dt_motor_torques
    info[2] = self._dt_motor_velocities
    ...
    if "PREJUMP_ONLY" in self._task_mode and self.get_sim_time() > FLIGHT_TIME_START:  # and sum(feetInContactBool) == 0
      ...
      while self.get_sim_time() > FLIGHT_TIME_START and not done:
        ...
        reward, terminate = self._reward_and_terminate()        # self._transform_action_to_motor_command(np.zeros(self._action_dim))
        q_des, q, dq_des, dq, tau = self._get_default_imitation_action()
        proc_action = self._opt_traj_pd_controller(q_des, q, dq_des, dq, tau)
    ...
    # print("reward:", reward)
    # print("done:", done)
    return np.array(self._noisy_observation()), reward, done, {'base_pos': self._robot.GetBasePosition()}
  ...
  def step(self, action):
    ...
    for _ in range(self._action_repeat): # 10,20,...
      ...
      elif self._isRLGymInterface: # true
    ...
    if "PREJUMP_ONLY" in self._task_mode and self.get_sim_time() > FLIGHT_TIME_START : # and sum(feetInContactBool) == 0
      ...
      while self.get_sim_time() > FLIGHT_TIME_START and not done:
        ...
        if "CARTESIAN" in self._task_mode:
          ...
          # Cartesian PD
          foot_pos, foot_vel, foot_force = self._get_cartesian_imitation_action()
          # print("foot_pos:", foot_pos) # 12
          # print("foot_vel:", foot_vel) #12
          # print("foot_force:", foot_force) #12
          tau = self._opt_traj_cartesian_pd_controller(foot_pos, foot_vel, foot_force)
          proc_action = tau + proc_action
    ...
    #reward = self._reward()
    #self._last_action = self._robot._applied_motor_torque
    reward, terminate = self._reward_and_terminate_v2()
    # print("terminate:", terminate)
    # print("get sim time:", self.get_sim_time())
    done = terminate or self.get_sim_time() > self._MAX_EP_LEN # also have max time
    #done = self.get_sim_time() > self._MAX_EP_LEN 
  ...
  def _reward_and_terminate(self):
    ...
    reward = (1 - reward - self._energy_weight * energy_reward) # survival bonus
  ...
  def _reward_and_terminate_v2(self):
    """ Get reward and terminate condition for imitation task.
    ...
    reward = self._base_pos_weight * np.linalg.norm(traj_base_pos - curr_base_pos) \
             ...
             + self._joint_pos_weight * np.linalg.norm(traj_joint_pos - curr_joint_pos) \
             + self._joint_vel_weight * np.linalg.norm(traj_joint_vel - curr_joint_vel) \
             + voltage_constraints_reward / (len(self._dt_motor_torques) * 12 * self._voltage_max) \
             + current_constraints_reward / (len(self._dt_motor_torques) * self._current_max)\
  ...
  def _get_observation(self):
    ...
    if self._observation_space_mode == "MOTION_IMITATION_EXTENDED_CONTACTS":
      ...
      #                        self._robot.GetExtendedObservation2UpperBound())  # NEED TO FIX DIMENSION if want to use for noise of foot contact states - CHUONG
      self._add_obs_noise = (np.random.normal(scale=self._observation_noise_stdev, size=len(self._robot_state)))

      # print(type(robot_state)) # list type
    elif self._observation_space_mode == "MOTION_IMITATION_EXTENDED2_CONTACTS_HISTORY": # We don't use it anymore
      self._robot_state = self._robot.GetExtendedObservation2_History() # CHUONG, get history
      # print(" length of robot state:", len(self._robot_state))
    ...
    self._observation = self._robot_state # 73*
  ...
  def get_traj_state_at_time(self, t):
    ...
    # print("traj_state length:", len(traj_state)) # 72
    # print("base_pos:", base_pos) # 3
    # print("base_orn:", base_orn) # 3
    # print("joint_pos:", joint_pos) #12
