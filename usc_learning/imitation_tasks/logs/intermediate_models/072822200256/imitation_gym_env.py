...
class ImitationGymEnv(quadruped_gym_env.QuadrupedGymEnv):
  ...
  def setupObsSpace(self):
    ...
    if self._task_env == "FULL_TRAJ":
      if self._observation_space_mode == "MOTION_IMITATION_EXTENDED2_CONTACTS":
        obs_high = np.concatenate((obs_high, self.traj_task_max_range)) # size is 73+72
        obs_low  = np.concatenate((obs_low,  self.traj_task_min_range))
      elif self._observation_space_mode == "MOTION_IMITATION_EXTENDED2_CONTACTS_HISTORY": # NOTE: It is not meaningful if considering FULL_TRAJ
        obs_high = np.concatenate((obs_high, np.tile(self.traj_task_max_range, self._sensory_hist_len)))  # Chuong: size is (73+72)*hist_len?
    ...
    if "CONTACTS" in self._observation_space_mode:
      if self._observation_space_mode == "MOTION_IMITATION_EXTENDED2_CONTACTS":
        obs_high = np.concatenate((obs_high, np.ones(4) + OBSERVATION_EPS)) # 73+72+4
        obs_low = np.concatenate((obs_low, np.zeros(4) - OBSERVATION_EPS))
      elif self._observation_space_mode == "MOTION_IMITATION_EXTENDED2_CONTACTS_HISTORY":
        obs_high = np.concatenate((obs_high, np.array([1, 1, 1, 1] * self._sensory_hist_len) + OBSERVATION_EPS)) # (73+72+4)*hist_len
  ...
  def step_full_traj(self,action):
    ...
    for i in range(self._task_env_num):
      for j in range(self._action_repeat): # check line 280
        ...
        if j % 10 == 0: # get torque and motor vel every 10ms
    ...
    if True: #"PREJUMP_ONLY" in self._task_mode: # and self.get_sim_time() > FLIGHT_TIME_START : # and sum(feetInContactBool) == 0
      ...
      final_estimate = self._get_terminal_reward_v2()
      #reward = total_reward
  ...
  ######################################################################################
  # penalty
  def uniform_penalty(self, x, x_max):
  ...
  def DP_penalty(self, x1, x2, rho):
    if (1/rho)*np.log(x1+x2)>0:
      return (1/rho)*np.log(x1+x2)
    else:
      return 0
  ...
  def _get_terminal_reward_v3(self):  # consider motor dynamic constraints in reward
    ...
    self._alpha_motor = (min_js - max_js) / (torque_motor_max - 0.2)
    self._voltage_max = 21.5
    self._current_max = 60
    ...
    energy_reward = 0
    voltage_constraints_reward = 0
    current_constraints_reward = 0
    voltage_constraints_value = 0
    current_constraints_value = 0
    DP_constraints_reward = 0

    # voltage = (0.5 * tau_joint(:, k) * alpha_motor / gear_ratio + dqjk * gear_ratio)*Kt;
    # current = ones(1, 4) * tau_joint(:, k) / (gear_ratio * Kt)

    # print("length:", len(self._dt_motor_torques))

    for i in range(len(self._dt_motor_torques)):
      for j in range(12):
        # print("get torque: ", self._dt_motor_torques[i][j])
        # print("get joint velocity: ", self._dt_motor_velocities[i][j])
        voltage = (0.5 * self._dt_motor_torques[i][j] * self._alpha_motor / self._gear_ratio +
                   self._dt_motor_velocities[i][j] * self._gear_ratio) * Kt
        # print("voltage: ", voltage)
        # voltage_constraints_reward += self.linear_penalty(voltage, self._voltage_max)
        voltage_constraints_value += math.exp(rho * (voltage - self._voltage_max))
        voltage_constraints_value += math.exp(rho * (-voltage - self._voltage_max))
      ...
      current_constraints_value += math.exp(rho * (current - self._current_max))
      current_constraints_value += math.exp(rho * (-current - self._current_max))

    voltage_constraints_value = voltage_constraints_value / (len(self._dt_motor_torques) * 12 * self._voltage_max)
    current_constraints_value = current_constraints_value / (len(self._dt_motor_torques) * self._current_max)

    DP_constraints_reward = mu * self.DP_penalty(current_constraints_value, voltage_constraints_value, rho)
  ...
  def _get_terminal_reward_v2(self): # consider motor dynamic constraints in reward
    ...
    reward = 100 * np.linalg.norm(traj_base_pos - curr_base_pos) \
             + 100 * np.linalg.norm(traj_base_orn - curr_base_orn) \
             + 100 * voltage_constraints_reward/(len(self._dt_motor_torques)*12*self._voltage_max) \
             + 100 * current_constraints_reward/(len(self._dt_motor_torques)*self._current_max)
    reward = 100 - reward  # survival bonus
  ...
  def _get_terminal_reward(self):
    curr_time = self.get_sim_time() # 1.3s
    # desired trajectory state at this time
    traj_base_pos, traj_base_orn, traj_base_linvel, \
    traj_base_angvel, traj_joint_pos, traj_joint_vel = self.get_traj_state_at_time(curr_time)
    # current robot state
  ...
  def _reward_and_terminate(self):
    ...
    energy_reward = 0
    torque_constraints_reward = 0

    for tau, vel in zip(self._dt_motor_torques, self._dt_motor_velocities):
      energy_reward += np.abs(np.dot(tau, vel)) * self._time_step
      # torque_constraints_reward += self.uniform_penalty(tau, self._joint_torque_max)

    # print("motor torque:", self._dt_motor_torques[1])
    # print("motor velocity:", self._dt_motor_velocities)
  ...
  def _get_observation(self):
    ...
    # current robot state
    if self._observation_space_mode == "MOTION_IMITATION_EXTENDED_CONTACTS":
      self._robot_state = self._robot.GetExtendedObservation()
    elif self._observation_space_mode == "MOTION_IMITATION_EXTENDED2_CONTACTS":
      ...
      # print("robot_state_size:", self._robot_state.shape)
    else:
    ...
    for obs_time in obs_times:
      ...
      # print('traj_desired_states shape:', traj_desired_states.shape)  #72

    if self._observation_space_mode == "MOTION_IMITATION_EXTENDED2_CONTACTS":
      self._observation = np.hstack((self._robot_state, traj_desired_states))
    elif self._observation_space_mode == "MOTION_IMITATION_EXTENDED2_CONTACTS_HISTORY":
      self._observation = np.hstack((self._robot_state, np.tile(traj_desired_states, self._sensory_hist_len)))
    else:
      raise ValueError('wrong obs mode')
    ...
    if "CONTACTS" in self._observation_space_mode:
      _, _, _, feetInContactBool = self._robot.GetContactInfo()
      if self._observation_space_mode == "MOTION_IMITATION_EXTENDED2_CONTACTS":
        self._observation = np.hstack(( self._observation, feetInContactBool))
      elif self._observation_space_mode == "MOTION_IMITATION_EXTENDED2_CONTACTS_HISTORY":
        self._observation = np.hstack((self._observation, feetInContactBool * self._sensory_hist_len)) # (73+72+4)*history_length
      else:
        raise ValueError('wrong obs mode')
