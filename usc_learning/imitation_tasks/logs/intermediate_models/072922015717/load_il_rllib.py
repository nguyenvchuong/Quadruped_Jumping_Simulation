...
from usc_learning.envs.quadruped_master import configs_a1
...
# new reference data
log_dir ='070822035050' # SAC , full-traj. 3 million steps
log_dir ='070822042545' # SAC, full-traj. 3 million steps
log_dir ='070922094855' # SAC, motor constraints
log_dir ='071022233312' # SAC, motor constraints with more training steps ---> Success!
# log_dir ='072422052410'
################################################################################################
...
class DummyQuadrupedEnv(gym.Env):
    ...
    def __init__(self, dummy_env_config):
        ...
        #self.env = ImitationGymEnv(observation_space_mode="MOTION_IMITATION_EXTENDED2_CONTACTS", task_env="FULL_TRAJ") # why it did not work
...
# TODO: save model params for mean_std_filter
...
write_out_path = 'exported_models/MLP/071022233312/'
...
# policy.model.save_model(write_out_path + 'my_model') # does not work

agent.export_policy_model(write_out_path + 'my_model') # CHUONG

# print("Model exported")

# import tensorflow as tf
# print(tf.__version__) # currently is TF2
...
for i in range(TEST_STEPS):
    ...
    # joint_taus[:,_] = env.env._robot.GetMotorTorques()
    # joint_vels[:,_] = env.env._robot.GetMotorVelocities()
    # joint_pos[:,_] = env.env._robot.GetMotorAngles()
    # print("joint_taus:", joint_taus[:, _])
    # print("obs value:", obs)
    ...
    if done:
        ...
        # print('info:', info)
        # df = pd.DataFrame.from_dict(info["torque"])
        # df.to_csv("torque_step_"+str(i) + ".csv", index=False, header=False)

        df = pd.DataFrame.from_dict(info["torques_velocities"])
        df.to_csv("torques_velocities_step_"+str(i) + ".csv", index=False, header=False)
        episode_reward = 0
        num_steps = 0
        obs = env.reset()
