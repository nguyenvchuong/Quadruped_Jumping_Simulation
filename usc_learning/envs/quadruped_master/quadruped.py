...
class Quadruped(object):
  ...
  def GetBasePosition_2D(self):
    """Get the position of quadruped's base in 2D, disregard y direction

    Returns:
      The position of quadruped's base.
    """
    position3D = self.GetBasePosition()
    position = np.array([position3D[0], position3D[2]])
  ...
  def GetBaseOrientation(self):
    ...
    # print("orientation observation:", type(orientation)) # tuple

    return orientation

  def GetBaseOrientation_2D(self):
    """Get the orientation of quadruped's base, pitch angle

    Returns:
      The orientation of quadruped's base.
    """
    orientation = self.GetBaseOrientation()
    roll_pitch_yaw = np.asarray(self._pybullet_client.getEulerFromQuaternion(orientation))
    orientation = np.array([roll_pitch_yaw[1]]) # get pitch only
    # print("orientation pitch is:", orientation)
  ...
  def GetTrueBaseRollPitchYawRate(self):
    ...
    orientation = self.GetBaseOrientation()
    RPY_rate = self.TransformAngularVelocityToLocalFrame(angular_velocity, orientation)
    return RPY_rate

  def GetTrueBasePitchRate(self):
    """Get the rate of orientation change of the minitaur's base in euler angle.

    Returns:
      rate of (pitch) change of the  base.
    """
    #angular_velocity = self._p.getBaseVelocity(self.quadruped)[1]
    #orientation = self.GetTrueBaseOrientation()
    angular_velocity = self.GetBaseAngularVelocity()
    orientation = self.GetBaseOrientation()
    RPY_rate = self.TransformAngularVelocityToLocalFrame(angular_velocity, orientation)
    pitch_rate = np.array([RPY_rate[1]])
    # print("rpy rate:", pitch_rate)
    return pitch_rate
  ...
  def GetBaseLinearVelocity_2D(self):
    ...
    linVel_2D = np.array([linVel[0], linVel[2]])
    return linVel_2D
  ...
  def Get2D(selfs, input):

    output = np.zeros(4)
    output[0]= input[1] # front thigh
    output[1]= input[2]# front calf
    output[2]= input[7]# rear thigh
    output[3]= input[8]  # rear thigh

    return output
    # rear calf
  def GetMotorAngles_2D(self):
    """Get quadruped motor angles at the current moment.
    ...
    motor_angles = self.Get2D(motor_angles)
  ...
  def GetMotorVelocities_2D(self):
    """Get the velocity of all motors.
    ...
    motor_velocities = self.Get2D(motor_velocities)
  ...
  def GetExtendedObservation_reduce(self):
    """ removed torque in observation
    """
    observation = []
    observation.extend(self.GetMotorAngles().tolist())
    observation.extend(self.GetMotorVelocities().tolist())
  ...
  def GetExtendedObservation_reduce_2D(self):
    """ removed torque in observation
    """
    observation = []
    observation.extend(self.GetMotorAngles_2D().tolist())
    observation.extend(self.GetMotorVelocities_2D().tolist())
    observation.extend(list(self.GetBaseOrientation_2D()))
    observation.extend(list(self.GetBasePosition_2D()))
    observation.extend(list(self.GetBaseLinearVelocity_2D()))
    observation.extend(list(self.GetTrueBasePitchRate()))
    return observation

  def GetExtendedObservation_reduce_v4(self):
    """ removed torque in observation
    """
    observation = []
    observation.extend(self.GetMotorAngles().tolist())
  ...
  def GetReferenceLowerBound(self):

    lower_bound = np.array([0.0]* 72)
    lower_bound[0:3] = (-2.,-2.,0.1)   # base position
    lower_bound[3:6] = (-1, -2, -1) # base orientation ?
    lower_bound[6:9] = (-10.,-10.,-10.) # base linear velocity
    lower_bound[9:12] = (-10., -10., -10.)  # base angle velocity
    lower_bound[12:24] = (self._robot_config.LOWER_ANGLE_JOINT)  # Joint angle.
    lower_bound[24:36] = (-self._robot_config.VELOCITY_LIMITS)  # Joint velocity.
    lower_bound[36:48] = np.array([-1, -1, -1] * 4)  # foot pos
    lower_bound[48:60] = np.array([-10, -10, -10] * 4)  # foot vel
    lower_bound[60:72] = np.array([-300, -200, -500]*4) # foot force
    return lower_bound

  def GetReferenceUpperBound(self):

    upper_bound = np.array([0.0] * 72)
    upper_bound[0:3] = (4.,2.,2.)  # base position
    upper_bound[3:6] = (1, 2, 1)  # base orientation ?
    upper_bound[6:9] = (10.,10.,10.) # base linear velocity
    upper_bound[9:12] = (10.,10.,10.) # base angle velocity
    upper_bound[12:24] = (self._robot_config.UPPER_ANGLE_JOINT)  # Joint angle.
    upper_bound[24:36] = (self._robot_config.VELOCITY_LIMITS)  # Joint velocity.
    upper_bound[36:48] = np.array([1, 1, 1] * 4)  # foot pos
    upper_bound[48:60] = np.array([10, 10, 10] * 4)  # foot vel
    upper_bound[60:72] = np.array([300, 200, 500] * 4)  # foot force

    return upper_bound

  def GetInitFinalConfigUpperBound(self):

    upper_bound = np.array([0.0] * 12)
    upper_bound[0:3] = (0.5,0.5,0.5)  # base position at start
    upper_bound[3:6] = (0.5, 0.5, 0.5)  # base orientation at start
    upper_bound[6:9] = (1,0.5,1) # base position at end
    upper_bound[9:12]= (0.5, 0.5, 0.5)  # base orientation at end

    return upper_bound

  def GetInitFinalConfigUpperBound_2D(self):

    upper_bound = np.array([0.0] * 4)
    upper_bound[0:2] = (0.5, 0.5)  # base position at start
    upper_bound[2:4] = (0.1, 0.1)  # box height

    return upper_bound

  def GetInitFinalConfigUpperBound_2D_posorn(self):

    upper_bound = np.array([0.0] * 3)
    upper_bound[0:2] = (0.5, 0.5)  # base position at start
    upper_bound[2] = 0.5  # box height

    return upper_bound



  def GetInitFinalConfigLowerBound(self):

    lower_bound = np.array([0.0] * 12)
    lower_bound[0:3] = (-0.5,-0.5,0)  # base position at start
    lower_bound[3:6] = (-0.5, -0.5, -0.5)  # base orientation at start
    lower_bound[6:9] = (-1,-0.5,0) # base position at end
    lower_bound[9:12] = (-0.5, -0.5, -0.5)  # base orientation at end

    return lower_bound

  def GetInitFinalConfigLowerBound_2D(self):

    lower_bound = np.array([0.0] * 4)
    lower_bound[0:2] = (-0.5, 0)  # base position at start
    lower_bound[2:4] = (0, 0)  # random box height

    return lower_bound

  def GetInitFinalConfigLowerBound_2D_posorn(self):

    lower_bound = np.array([0.0] * 3)
    lower_bound[0:2] = (-0.5, 0)  # base position at start
    lower_bound[2] = -0.5  # random box height
  ...
  def GetExtendedObservationUpperBound_reduce(self):
    """Get the upper bound of the observation.
    Note: the upper/lower bounds are going to be larger than actual
    ...
    Returns:
      The upper bound of an observation. See GetExtendedObservation() for the details
        of each element of an observation.
    """
    upper_bound = np.array([0.0] * len(self.GetExtendedObservation_reduce()))
    upper_bound[0:self.num_motors] = (self._robot_config.UPPER_ANGLE_JOINT)  # Joint angle.
    upper_bound[self.num_motors:2 * self.num_motors] = (self._robot_config.VELOCITY_LIMITS)  # Joint velocity.
    upper_bound[2 * self.num_motors:2 * self.num_motors+4] = 1.0  # Quaternion of base orientation.
    upper_bound[2 * self.num_motors+4:2 * self.num_motors+4+3] = (4.,2.,2.) # base pos max
    upper_bound[2 * self.num_motors+4+3:2 * self.num_motors+4+6] = (10.,10.,10.) # base lin vel max
    upper_bound[2 * self.num_motors+4+6:2 * self.num_motors+4+9] = (10.,10.,10.) # base ang vel max
    return upper_bound

  def GetExtendedObservationUpperBound_reduce_2D(self):
    """Get the upper bound of the observation.
    Note: the upper/lower bounds are going to be larger than actual
    ...
    Returns:
      The upper bound of an observation. See GetExtendedObservation() for the details
        of each element of an observation.
    """
    upper_bound = np.array([0.0] * len(self.GetExtendedObservation_reduce_2D()))
    upper_bound[0:4] = np.array([4.18879020479, -0.916297857297] * 2)  # Joint angle.
    upper_bound[4:8] = np.asarray([21.0] * 4)   # Joint velocity.
    upper_bound[8:9] = 3.14  # pitch angle
    upper_bound[9:11] = (2.,2.) # base pos max
    upper_bound[11:13] = (10.,10.) # base lin vel max
    upper_bound[13] = 10. # base ang vel max
    return upper_bound


  def GetExtendedObservationUpperBound_reduce_v4(self):
    """Get the upper bound of the observation.
    Note: the upper/lower bounds are going to be larger than actual
  ...
  def GetExtendedObservationLowerBound_reduce(self):
    """Get the lower bound of the observation."""
    lower_bound = np.array([0.0] * len(self.GetExtendedObservation_reduce()))
    lower_bound[0:self.num_motors] = (self._robot_config.LOWER_ANGLE_JOINT)  # Joint angle.
    lower_bound[self.num_motors:2 * self.num_motors] = (-self._robot_config.VELOCITY_LIMITS)  # Joint velocity.
    lower_bound[2 * self.num_motors:2 * self.num_motors+4] = -1.0  # Quaternion of base orientation.
    lower_bound[2 * self.num_motors+4:2 * self.num_motors+4+3] = (-2.,-2.,0.1) # base pos min
    # lower_bound[3 * self.num_motors+4+3:3 * self.num_motors+4+3+3] = (-4.,-4.,-4.) # base lin vel min
    # lower_bound[3 * self.num_motors+4+6:3 * self.num_motors+4+9] = (-4.,-4.,-4.) # base ang vel min
    lower_bound[2 * self.num_motors+4+3:2 * self.num_motors+4+3+3] = (-10.,-10.,-10.) # base lin vel min
    lower_bound[2 * self.num_motors+4+6:2 * self.num_motors+4+9] = (-10.,-10.,-10.) # base ang vel min
    return lower_bound

  def GetExtendedObservationLowerBound_reduce_2D(self):
    """Get the lower bound of the observation."""
    lower_bound = np.array([0.0] * len(self.GetExtendedObservation_reduce_2D()))
    lower_bound[0:4] = np.array([-1.0471975512 , -2.69653369433] * 2)  # Joint angle.
    lower_bound[4:8] = -np.array([21.0] * 4)  # Joint velocity.
    lower_bound[8:9] = -3.14  # Quaternion of base orientation.
    lower_bound[9:11] = (-2.,0.1) # base pos min
    lower_bound[11:13] = (-10.,-10) # base lin vel min
    lower_bound[13] = -10. # base ang vel min
    return lower_bound
  ...
  def GetExtendedObservation_v2(self):
    """Get the full observation of quadruped, includes body positions and velocities.
    ...
    Returns:
      The observation list.
      observation[] are motor angles.
      observation[] are motor velocities,
      observation[] is the orientation of the base, in quaternion form.
      observation[] is the base position
      observation[] is base linear velocity
      observation[] is base angular velocity
    """
    observation = self.GetExtendedObservation_reduce()
    # print("observation in:", len(observation)) # 49
    # add foot pos/ foot vel
    for i in range(4):
      foot_pos = self.GetLocalEEPos(i)
    ...
    # print("observation size:", len(observation))
    return observation # size now is 37+ 24= 61

  def GetExtendedObservation_v2_2D(self):
    """Get observation for 2D motion
    ...
    """
    observation = self.GetExtendedObservation_reduce_2D()
    # print("observation in:", len(observation)) #
    # add foot pos/ foot vel
    for i in range(2):
      idx = 2*i
      foot_pos = self.GetLocalEEPos(idx)
      foot_pos_2D = np.array([foot_pos[0], foot_pos[2]])
      # Jacobian and current foot position in hip frame
      J, ee_pos_legFrame = self._ComputeJacobianAndPositionUSC(idx)
      # foot velocity in leg frame
      foot_linvel = J @ self.GetMotorVelocities()[idx*3:idx*3+3]
      foot_linvel_2D = np.array([foot_linvel[0], foot_linvel[2]])
      observation.extend(tuple(foot_pos_2D)) # 4
      observation.extend(tuple(foot_linvel_2D)) # 4

    # print("observation size:", len(observation))
    return observation # size now is 14+ 8= 22

  def GetExtendedObservation_v4(self):
    """Get the full observation of quadruped, includes body positions and velocities.
    ...
    Returns:
      ...
      observation[28:31] is the base position
      observation[31:34] is base linear velocity
      observation[34:37] is base angular velocity
    """
    observation = self.GetExtendedObservation_reduce_v4()
    # print("observation in:", len(observation)) # 49
    # add foot pos/ foot vel
    # for i in range(4):
    #   foot_pos = self.GetLocalEEPos(i)
    ...
    # print("observation size:", len(observation))
    return observation # size now is 37= 37
  ...
  def GetExtendedObservation2_History(self, **kwargs):
    ...
    sensor_observation = np.concatenate(self._sensor_records) # 74*hist_len
    # print("sensor observation size:", sensor_observation.shape)
    return sensor_observation

    # return base_observation
  def GetExtendedObservation2_History_Contact(self, **kwargs):

    base_observation = self.GetExtendedObservation2() # has size of 73
    _, _, _, feetInContactBool = self.GetContactInfo()
    # print("feetInContactBool:", feetInContactBool)
    base_observation = np.append(base_observation, feetInContactBool) # has size of 77

    if self._sensor_records is None:
      self._sensor_records = [np.zeros(77)] * self._sensory_hist_len
    self._sensor_records.pop(0)
    self._sensor_records.append(base_observation)
    # sensor_observation = np.stack(self._sensor_records, axis=0).flatten()
    sensor_observation = np.concatenate(self._sensor_records) # 77*hist_len
    # print("sensor observation size:", sensor_observation.shape)
    return sensor_observation


  def GetExtendedObservation2UpperBound_Reference(self):
    """Get the upper bound of the observation.
    Note: the upper/lower bounds are going to be larger than actual
    ...
    upper_bound = np.concatenate((upper_bound, np.array([1, 1, 1, 1])))
    upper_bound = np.concatenate((upper_bound, np.array([10]*72)))

    # print("upper_bound size:", upper_bound.shape) # 73
    return upper_bound

  def GetExtendedObservation2LowerBound_Reference(self):
    """Get the lower bound of the observation."""
    lower_bound = self.GetExtendedObservationLowerBound() #49
    lower_bound = np.concatenate(( lower_bound,
                                  np.array([-1,-1,-1,-10,-10,-10]*4 )))
    # print("upper_bound size", lower_bound.shape) # 73
    lower_bound = np.concatenate((lower_bound, np.array([0, 0, 0, 0]))) # 77
    lower_bound = np.concatenate((lower_bound, np.array([-10] * 72))) # 149
    return lower_bound
  ...
  def GetExtendedObservation2UpperBound_v2(self):
    """Get the upper bound of the observation.
    Note: the upper/lower bounds are going to be larger than actual
    ...
    Returns:
      The upper bound of an observation. See GetExtendedObservation() for the details
        of each element of an observation.
    """
    upper_bound = self.GetExtendedObservationUpperBound_reduce() # size is 37
    # print("upper_bound size:", upper_bound.shape) # 37
    # fake bounds, need to calculate
    upper_bound = np.concatenate(( upper_bound,
                                  np.array([1,1,1,10,10,10]*4 ))) # size is 37+24=61

    # print("upper_bound size:", upper_bound.shape) # 73
    return upper_bound

  def GetExtendedObservation2UpperBound_v2_2D(self):
    """Get the upper bound of the observation.
    Note: the upper/lower bounds are going to be larger than actual
    ...
    Returns:
      The upper bound of an observation. See GetExtendedObservation() for the details
        of each element of an observation.
    """
    upper_bound = self.GetExtendedObservationUpperBound_reduce_2D() #
    # print("upper_bound size:", upper_bound.shape) #
    # fake bounds, need to calculate
    upper_bound = np.concatenate(( upper_bound,
                                  np.array([1,1,10,10]*2 ))) # size is ?+24=

    # print("upper_bound size:", upper_bound.shape) # 73
    return upper_bound


  def GetExtendedObservation2UpperBound_v4(self):
    """Get the upper bound of the observation.
    Note: the upper/lower bounds are going to be larger than actual
  ...
  def GetExtendedObservation2LowerBound_v2(self):
    """Get the lower bound of the observation."""
    lower_bound = self.GetExtendedObservationLowerBound_reduce()
    lower_bound = np.concatenate(( lower_bound,
                                  np.array([-1,-1,-1,-10,-10,-10]*4 )))
    # print("upper_bound size", lower_bound.shape) # 61
    return lower_bound
  def GetExtendedObservation2LowerBound_v2_2D(self):
    """Get the lower bound of the observation."""
    lower_bound = self.GetExtendedObservationLowerBound_reduce_2D()
    lower_bound = np.concatenate(( lower_bound,
  ...
  def GetExtendedObservation2LowerBound_v4(self):
    """Get the lower bound of the observation."""
    lower_bound = self.GetExtendedObservationLowerBound_reduce()
    # lower_bound = np.concatenate(( lower_bound,
    #                               np.array([-1,-1,-1,-10,-10,-10]*4 )))
  ...
  def GetExtendedObservation2UpperBound_History_Contact(self):
    """Get the upper bound of the observation.
    Note: the upper/lower bounds are going to be larger than actual
    ...
    upper_bound = np.concatenate(( upper_bound,
                                  np.array([1,1,1,10,10,10]*4 ))) # size is 49+24=73
    upper_bound = np.concatenate((upper_bound, np.array([1,1,1,1]))) # size is 73+4=77
  ...
  def GetExtendedObservation2LowerBound_History_Contact(self):
    """Get the lower bound of the observation."""
    lower_bound = self.GetExtendedObservationLowerBound()
    lower_bound = np.concatenate(( lower_bound,
                                  np.array([-1,-1,-1,-10,-10,-10]*4 )))

    lower_bound = np.concatenate((lower_bound, np.array([0, 0, 0, 0])))  # size is 73+4=77
    lower_bound = [lower_bound] * self._sensory_hist_len
    lower_bound = np.concatenate(lower_bound)
  ...
  def ApplyAction(self, motor_commands):
    ...
    """
    # print("motor_commands 1:", motor_commands)
  ...
  def ApplyAction_v2(self, motor_commands): # With motor dynamics constraints
    ...
    Args:
      motor_commands: The total motor torque computed from RL
    """
    if self._motor_velocity_limit < np.inf:
      current_motor_angle = self.GetMotorAngles()
      motor_commands_max = (current_motor_angle + self.time_step * self._motor_velocity_limit)
    ...
    if self._accurate_motor_model_enabled or self._pd_control_enabled:
      ...
      # print('MOTOR COMMANDS \n', motor_commands, '\n')
      if self._accurate_motor_model_enabled:
        actual_torque, observed_torque = self._motor_model.convert_to_torque_v2(
          motor_commands, q, qdot)
  ...
  def _ComputeJacobianAndPositionUSC_2D(self, legID, specific_delta_q=None):
    """Translated to python/pybullet from:
    https://github.com/DRCL-USC/USC-AlienGo-Software/blob/master/laikago_ros/laikago_gazebo/src/LegController.cpp
    can also use pybullet function calculateJacobian()
    IN LEG FRAME
    ...
    l1 = self._robot_config.HIP_LINK_LENGTH*0
    l2 = self._robot_config.THIGH_LINK_LENGTH
    l3 = self._robot_config.CALF_LINK_LENGTH
