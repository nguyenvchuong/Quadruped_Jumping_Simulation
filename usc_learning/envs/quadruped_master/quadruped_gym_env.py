...
class QuadrupedGymEnv(gym.Env):
  ...
  def __init__(
      ...
      time_step=0.001,
      action_repeat= 200, # 50 seems to be working, couldn't verify though
      distance_weight=2.0,
      energy_weight=0.008,#05,#0.005,#0.005, # orig 0.1
      shake_weight=0,#1e-4,
      drift_weight=0,#1e-4,
      distance_limit=float("inf"),
      observation_noise_stdev =0.05*0,
  ...
  def _transform_action_to_motor_command(self, action):
    ...
    if self._motor_control_mode == "PD":
      ...
      action = self._robot.ScaleActionToIKRange(action)    # TODO: fix, this is not clean
  ...
  def add_box_each_rear_foot(self, z_height):
    """ add box under each foot, temp test """
    box_x = 0.2
    box_y = 0.25
    # RR
